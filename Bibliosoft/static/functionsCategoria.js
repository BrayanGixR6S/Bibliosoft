...
$(function() {
    ...
    $.ajax({
        url:'/libro/categoria/data',
        type: 'GET',
        success: function(response){
            console.log(response);
            const table = new DataTable('#dataCategoria', {
                "data":response['datos'],
                "columns": [
                    {"data":response[0]},
                    {"data":response[1]},
            ...
            //*SELECT
            $('#dataCategoria tbody').on('click','tr', function() {
                var rowIndex = table.row(this).index();
                var idRow = $('#dataCategoria').DataTable().cell(rowIndex,0).data();
...
$("#updateForm").submit(function(event) {
    ...
    $.ajax({
        url: "/libro/categoria/update/"+idRow+"/",
        type: "post",
        data: {
            nombre: nombre,
            estatus: estatus,
