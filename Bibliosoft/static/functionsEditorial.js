...
$(function() {
    ...
    $.ajax({
        url:'/libro/editorial/data',
        type: 'GET',
        success: function(response){
            console.log(response);
            const table = new DataTable('#dataEditorial', {
                "data":response['datos'],
                "columns": [
                    {"data":response[0]},
                    {"data":response[1]},
            ...
            //*SELECT
            $('#dataEditorial tbody').on('click','tr', function() {
                var rowIndex = table.row(this).index();
                var idRow = $('#dataEditorial').DataTable().cell(rowIndex,0).data();
...
$("#updateForm").submit(function(event) {
    ...
    $.ajax({
        url: "/libro/editorial/update/"+idRow+"/",
        type: "post",
        data: {
            nombre: nombre,
            estatus: estatus,
